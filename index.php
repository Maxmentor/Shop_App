...
if (isset($_SESSION['admin'])) {
    ...
    if (isset($_POST['process_sale'])) {
        ...
        if ($prod && $prod['quantity'] >= $qty_sold) {
            ...
            $cust_mob = ($_POST['cust_type'] == 'Custom') ? $_POST['cust_mobile'] : 'N/A';
...
<head>
    ...
    <style>
        ...
        body { background: #f8fafc; font-family: 'Segoe UI', system-ui, -apple-system, sans-serif; color: #334155; display: flex; flex-direction: column; min-height: 100vh; }
        .sidebar { background: var(--sidebar-bg); min-height: 100vh; color: #fff; box-shadow: 4px 0 10px rgba(0,0,0,0.05); }
        .sidebar a { color: #94a3b8; text-decoration: none; padding: 12px 20px; display: block; border-radius: 8px; margin: 6px 12px; font-weight: 500; transition: all 0.2s; }
        ...
        .main-footer { background: #ffffff; border-top: 1px solid #e2e8f0; font-size: 14px; margin-top: auto; }
        @media print {
...
<?php if (!isset($_SESSION['admin'])): ?>
    <div class="container d-flex justify-content-center align-items-center min-vh-100 flex-grow-1">
        <div class="card card-box p-4 p-md-5" style="width: 420px;">
...
<?php else: ?>
        ...
        <div class="row">
            <div class="col-md-3 col-lg-2 px-0 sidebar no-print">
                <div class="p-4 text-center border-bottom border-secondary border-opacity-25">
                    <h5 class="fw-bold text-white m-0"><i class="fas fa-box-archive text-primary me-2"></i>STOCK APP</h5>
                </div>
                <nav class="mt-3">
                    ...
                    <a href="index.php?logout=true" class="text-danger mt-4"><i class="fas fa-right-from-bracket me-2"></i> Logout</a>
                </nav>
            </div>

            <div class="col-md-9 col-lg-10 ms-sm-auto px-md-4 py-4 d-flex flex-column justify-content-between" style="min-height: 100vh;">
                <div>
                <?php 
                // ==========================================
                // DASHBOARD
                ...
                ?>
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h2>Dashboard Overview & Profit Analysis</h2>
                    ...
                    <div class="row g-4">
                        <div class="col-md-3">
                            <div class="card card-box p-4 border-start border-secondary border-4">
                                <span class="text-muted small fw-bold">TOTAL PURCHASE INVESTMENT</span>
                                <h3 class="fw-bold mt-1 text-secondary">₹<?= number_format($total_purchase_investment, 2); ?></h3>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="card card-box p-4 border-start border-primary border-4">
                                <span class="text-muted small fw-bold">TOTAL INVENTORY VALUE</span>
                                <h3 class="fw-bold mt-1 text-primary">₹<?= number_format($inv_summary['total_inv'] ?? 0, 2); ?></h3>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="card card-box p-4 border-start border-success border-4">
                                <span class="text-muted small fw-bold">TOTAL SALE INCOME</span>
                                <h3 class="fw-bold mt-1 text-success">₹<?= number_format($total_sale, 2); ?></h3>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="card card-box p-4 border-start border-warning border-4">
                                <span class="text-muted small fw-bold">NET PROFIT</span>
                                <h3 class="fw-bold mt-1 <?= $net_profit >= 0 ? 'text-success' : 'text-danger'; ?>">₹<?= number_format($net_profit, 2); ?></h3>
                            </div>
                        </div>
                    </div>
                    <div class="row g-4 mt-1">
                        <div class="col-md-3">
                            <div class="card card-box p-4 border-start border-danger border-4">
                                <span class="text-muted small fw-bold">OUT OF STOCK</span>
                                <h3 class="fw-bold mt-1 text-danger"><?= $out_stock_count; ?></h3>
                            </div>
                        </div>
                    </div>
                ...
                ?>
                    ...
                    <div class="card card-box p-4 mt-3 col-md-8">
                        <form method="POST">
                            <div class="mb-3"><label class="form-label fw-bold">Shop Name</label><input type="text" name="name" class="form-control" value="<?= $shop['name']; ?>" required></div>
                            <div class="mb-3"><label class="form-label fw-bold">Mobile Number</label><input type="text" name="mobile" class="form-control" value="<?= $shop['mobile']; ?>" required></div>
                            <div class="mb-3"><label class="form-label fw-bold">Website</label><input type="text" name="website" class="form-control" value="<?= $shop['website']; ?>"></div>
                            <div class="mb-3"><label class="form-label fw-bold">GST Number</label><input type="text" name="gst" class="form-control" value="<?= $shop['gst']; ?>"></div>
                            <div class="mb-3"><label class="form-label fw-bold">Address</label><textarea name="address" class="form-control" rows="3"><?= $shop['address']; ?></textarea></div>
                            <button type="submit" name="update_shop" class="btn btn-primary px-4 fw-bold">Update Details</button>
                        </form>
                    </div>

                <?php 
                // ==========================================
                // ADMIN MANAGER (Change Username & Password)
                // ==========================================
                elseif ($page == 'admin_manager'):
                    $admin_info = $pdo->query("SELECT * FROM admin WHERE id=1")->fetch();
                ?>
                    <h2>Admin Credentials Manager</h2>
                    <?php 
                    ...
                    ?>
                    <div class="card card-box p-4 mt-3 col-md-6">
                        <form method="POST">
                            <div class="mb-3">
                            ...
                            <button type="submit" name="update_admin" class="btn btn-primary px-4 fw-bold"><i class="fas fa-save me-1"></i> Update Credentials</button>
                        </form>
                    </div>

                <?php 
                // ==========================================
                // SUPPLIERS & SUPPLIER ITEMS VIEW
                // ==========================================
                ...
                ?>
                    ...
                    ?>
                        <div class="card card-box p-4 mb-4 bg-light border">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <h4 class="text-primary mb-0"><i class="fas fa-boxes me-2"></i>Items from Supplier: <?= $sup_data['name'] ?? $sup_code_items; ?> (Code: <?= $sup_code_items; ?>)</h4>
                                <a href="index.php?page=suppliers" class="btn btn-sm btn-secondary">Back to Suppliers</a>
                            </div>
                    ...
                    <div class="row mt-3">
                        ...
                        <div class="col-md-8">
                            <div class="card card-box p-3">
                                <h5 class="fw-bold mb-3">Supplier List</h5>
                                <table class="table align-middle">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td><?= $s['mobile']; ?></td>
                                            <td>
                                                <a href="index.php?page=suppliers&sup_code_items=<?= $s['sup_code']; ?>" class="btn btn-sm btn-info text-white me-1" title="View all items purchased from this supplier"><i class="fas fa-box-open"></i> All Items</a>
                            ...
                            <div class="card card-box p-3">
                                ...
                                <ul class="list-group list-group-flush">
                                    ...
                                    <li class="list-group-item d-flex justify-content-between align-items-center py-3">
                                        ...
                                        <div>
                                            <a href="index.php?page=category_products&cat_id=<?= $c['id']; ?>" class="btn btn-sm btn-info text-white me-1">View Products</a>
                ...
                ?>
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h2>Products in Category: <span class="text-primary"><?= $cat_title; ?></span></h2>
                        <a href="index.php?page=categories" class="btn btn-sm btn-secondary">Back to Categories</a>
                    </div>
                    <div class="card card-box p-3">
                        <table class="table align-middle">
                            <thead><tr><th>Image</th><th>Code</th><th>Product Name</th><th>Stock</th><th>Price</th></tr></thead>
                            <tbody>
                                <?php
                ...
                <?php 
                // ==========================================
                // ALL BILLS LIST & MANAGEMENT
                // ==========================================
                elseif ($page == 'all_bills_list'):
                    $search = $_GET['search'] ?? '';
                ?>
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h2>All External Bills Management</h2>
                        <a href="index.php?page=add_all_bill" class="btn btn-primary"><i class="fas fa-plus me-1"></i> Upload New Bill</a>
                    </div>
                    ...
                    <div class="card card-box p-3 mb-3">
                        <form method="GET" class="row g-2">
                            ...
                            <div class="col-md-6">
                                <input type="text" name="search" class="form-control" placeholder="Search by Name, GST or Mobile..." value="<?= $search; ?>">
                            </div>
                            <div class="col-md-2">
                            ...
                            <tbody>
                                ...
                                if(count($bills) > 0) {
                                    foreach($bills as $b) {
                                        ...
                                        echo "<tr>";
                                        echo "<td>#{$b['id']}</td>";
                                        echo "<td><small><i class='fas fa-calendar-alt me-1'></i>{$b['created_at']}</small></td>";
                                        ...
                                        foreach($imgs as $img) {
                                            if(!empty($img)) {
                                                ...
                                                echo "<img src='$img' width='35' height='35' class='rounded border shadow-sm' style='object-fit: cover;'>";
                                                echo "</a>";
                                            }
                                        }
                                        echo "</td>";
                                        echo "<td>";
                                        echo "<button class='btn btn-sm btn-info text-white me-1' onclick='viewBillImgs(" . json_encode($imgs) . ")'><i class='fas fa-eye'></i> View</button>";
                                        echo "<a href='index.php?page=all_bills_list&delete_bill={$b['id']}' class='btn btn-sm btn-outline-danger' onclick='return confirm(\"Delete this bill record?\")'><i class='fas fa-trash'></i></a>";
                                        echo "</td>";
                                        echo "</tr>";
                                    }
                                } else {
                        ...
                        <div class="modal-dialog modal-dialog-centered modal-lg">
                            <div class="modal-content bg-white shadow-lg border-0 p-3">
                                <div class="modal-header">
                                    <h5 class="fw-bold">Bill Images Preview & Download</h5>
                    ...
                    function viewBillImgs(images) {
                        ...
                        images.forEach(imgUrl => {
                            if(imgUrl.trim() !== '') {
                                ...
                                div.className = 'text-center border p-2 rounded shadow-sm bg-light';
                                div.innerHTML = `
                ...
                ?>
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <a href="index.php?page=all_bills_list" class="btn btn-secondary">Back to List</a>
                    </div>
                    <div class="card card-box p-4 col-md-8 mx-auto">
                        <form method="POST" enctype="multipart/form-data">
                            <div class="mb-3">
                                <label class="form-label fw-bold">Bill Name / Description</label>
                            ...
                            <div class="mb-3">
                                <label class="form-label fw-bold">Mobile Number (Optional)</label>
                                <input type="text" name="mobile" class="form-control" placeholder="Enter Mobile Number">
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-bold">Upload Multiple Bill Images</label>
                                <input type="file" name="bill_images[]" class="form-control" multiple required>
                                <small class="text-muted">You can select multiple images at once (Ctrl + Click or drag selection).</small>
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-bold">Date & Time</label>
                                <input type="text" class="form-control bg-light" value="<?= date('Y-m-d H:i:s'); ?> (Automatic)" disabled>
                            </div>
                            <button type="submit" name="add_external_bill" class="btn btn-primary w-100 py-2 fw-bold">Save Bill & Upload Images</button>
                        </form>
                    </div>

                <?php 
                // ==========================================
                // INVENTORY (With Select2 for Category & Supplier)
                // ==========================================
                ...
                ?>
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addInvModal"><i class="fas fa-plus me-1"></i> Add Product</button>
                    </div>
                    ...
                    <div class="card card-box p-3 mb-3">
                        <form method="GET" class="row g-2">
                            ...
                            <div class="col-md-6">
                                <input type="text" name="search" class="form-control" placeholder="Search by Product Name or Code..." value="<?= $search; ?>">
                            </div>
                            <div class="col-md-2">
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-4">
                            <div class="card card-box p-3 bg-light border">
                                <span class="text-muted small fw-bold">TOTAL FILTERED STOCK QTY</span>
                                <h5 class="fw-bold m-0 text-dark"><?= number_format($inv_totals['total_qty'] ?? 0); ?> Units</h5>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="card card-box p-3 bg-light border">
                                <span class="text-muted small fw-bold">FILTERED PURCHASE VALUE</span>
                                <h5 class="fw-bold m-0 text-primary">₹<?= number_format($inv_totals['total_pur_val'] ?? 0, 2); ?></h5>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="card card-box p-3 bg-light border">
                                <span class="text-muted small fw-bold">FILTERED SALE VALUE</span>
                                <h5 class="fw-bold m-0 text-success">₹<?= number_format($inv_totals['total_sale_val'] ?? 0, 2); ?></h5>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card card-box p-3">
                        <table class="table table-hover align-middle">
                            ...
                            <tbody>
                                ...
                                ?>
                                <tr>
                                    <td><img src="<?= $item['image'] ?: 'https://via.placeholder.com/40'; ?>" width="45" height="45" class="product-img shadow-sm" onclick="showFullImg('<?= $item['image'] ?: 'https://via.placeholder.com/400'; ?>')"></td>
                                    ...
                                    <td>
                                        <button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#editInvModal<?= $item['id']; ?>"><i class="fas fa-edit"></i></button>
                                        <a href="index.php?page=inventory&delete_inv=<?= $item['id']; ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Delete product?')"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                    ...
                    <div class="modal fade" id="editInvModal<?= $item['id']; ?>" tabindex="-1">
                        <div class="modal-dialog">
                            <form method="POST" enctype="multipart/form-data" class="modal-content bg-white shadow-lg border-0">
                                ...
                                <div class="modal-header"><h5 class="fw-bold">Edit Product: <?= $item['product_code']; ?></h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                                <div class="modal-body text-start">
                                    ...
                                    <div class="mb-2">
                                        <label class="form-label small">Category (Type to search)</label>
                                    ...
                                    <div class="mb-2">
                                        <label class="form-label small">Supplier Code (Type to search)</label>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-2"><label class="form-label small">Quantity</label><input type="number" name="quantity" class="form-control" value="<?= $item['quantity']; ?>" required></div>
                                        <div class="col-md-6 mb-2"><label class="form-label small">Unit</label>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-2"><label class="form-label small">Purchase Price</label><input type="number" step="0.01" name="purchase_price" class="form-control" value="<?= $item['purchase_price']; ?>" required></div>
                                        <div class="col-md-6 mb-2"><label class="form-label small">Sale Price</label><input type="number" step="0.01" name="sale_price" class="form-control" value="<?= $item['sale_price']; ?>" required></div>
                                    </div>
                                    <div class="mb-2"><label class="form-label small">New Image (Optional)</label><input type="file" name="image" class="form-control"></div>
                                </div>
                                <div class="modal-footer"><button type="submit" name="edit_inventory" class="btn btn-primary">Save Changes</button></div>
                            </form>
                        </div>
                    </div>
                    <?php endforeach; ?>

                    <!-- Add Inventory Modal with Select2 Live Search for Category & Supplier -->
                    <div class="modal fade" id="addInvModal" tabindex="-1">
                        <div class="modal-dialog">
                            <form method="POST" enctype="multipart/form-data" class="modal-content bg-white shadow-lg border-0">
                                <div class="modal-header"><h5 class="fw-bold">Add Inventory Product</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                                <div class="modal-body text-start">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-2"><label class="form-label small">Quantity</label><input type="number" name="quantity" class="form-control" required></div>
                                        <div class="col-md-6 mb-2"><label class="form-label small">Unit</label>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-2"><label class="form-label small">Purchase Price</label><input type="number" step="0.01" name="purchase_price" class="form-control" required></div>
                                        <div class="col-md-6 mb-2"><label class="form-label small">Sale Price</label><input type="number" step="0.01" name="sale_price" class="form-control" required></div>
                                    </div>
                                    <div class="mb-2"><label class="form-label small">Product Image</label><input type="file" name="image" class="form-control"></div>
                                </div>
                                <div class="modal-footer"><button type="submit" name="add_inventory" class="btn btn-primary">Save Product</button></div>
                            </form>
                        </div>
                    </div>
                ...
                elseif ($page == 'sales'):
                    if(isset($_GET['invoice'])):
                        ...
                        $bill = $sale->fetch();
                ?>
                    <div class="card card-box p-5 col-md-8 mx-auto shadow-lg" id="billContainer">
                        <div class="text-center border-bottom pb-4 mb-4">
                            <h2 class="fw-bold text-uppercase text-primary mb-1"><?= $bill['name']; ?></h2>
                            <p class="text-muted mb-1"><?= $bill['address']; ?></p>
                            <p class="text-muted mb-1">Mobile: <?= $bill['mobile']; ?> | Website: <?= $bill['website']; ?></p>
                            <p class="fw-bold m-0">GSTIN: <?= $bill['gst']; ?></p>
                        </div>
                        <div class="row mb-4">
                            <div class="col-6">
                                <h6 class="text-muted small fw-bold">INVOICE DETAILS</h6>
                                <p class="mb-1"><b>Invoice No:</b> <?= $bill['invoice_no']; ?></p>
                                <p class="mb-0"><b>Date:</b> <?= $bill['sale_date']; ?></p>
                            </div>
                            <div class="col-6 text-end">
                                <h6 class="text-muted small fw-bold">CUSTOMER DETAILS</h6>
                                <p class="mb-1"><b>Name:</b> <?= $bill['customer_name']; ?></p>
                                <p class="mb-1"><b>Mobile:</b> <?= $bill['customer_mobile']; ?></p>
                                <p class="mb-0"><b>Address:</b> <?= $bill['customer_address']; ?></p>
                            </div>
                        </div>
                        <table class="table table-bordered align-middle">
                            <thead class="table-light"><tr><th>Item Code & Name</th><th>Qty</th><th>Unit</th><th>Amount</th></tr></thead>
                            <tbody>
                                <tr>
                        ...
                        <div class="row justify-content-end">
                            <div class="col-md-5">
                                <table class="table table-sm table-borderless">
                                    ...
                                    <tr class="border-top fw-bold fs-5"><td>Grand Total:</td><td class="text-end text-primary">₹<?= number_format($bill['total_amount'], 2); ?></td></tr>
                                </table>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 text-center no-print">
                            <button onclick="window.print()" class="btn btn-success px-4 fw-bold me-2"><i class="fas fa-print me-1"></i> Print / Download Bill</button>
                            <a href="index.php?page=sales" class="btn btn-outline-primary px-4 fw-bold">New Sale</a>
                        </div>
                    </div>
                    <?php else: ?>
                    <h2>New Sale Counter</h2>
                    <?php if(isset($sale_error)) echo "<div class='alert alert-danger mt-3'>$sale_error</div>"; ?>
                    <div class="card card-box p-4 col-md-8 mt-3">
                        <form method="POST">
                            <div class="mb-3">
                            ...
                            <div id="customCustFields" style="display:none;" class="p-3 bg-light rounded mb-3 border">
                                ...
                                <div class="mb-2"><label class="form-label small">Mobile</label><input type="text" name="cust_mobile" class="form-control"></div>
                                <div class="mb-2"><label class="form-label small">Address</label><textarea name="address" class="form-control" rows="2"></textarea></div>
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-bold">Search & Select Product</label>
                                <select name="product_id" id="productSelect" class="form-control select2" required>
                                    ...
                                    foreach($prods as $p) {
                                        echo "<option value='{$p['id']}'>[Code: {$p['product_code']}] {$p['product_name']} | Stock: {$p['quantity']} {$p['unit']} | Price: ₹{$p['sale_price']} | Size: {$p['size']}</option>";
                                    }
                                    ?>
                                </select>
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-bold">Quantity Sold</label>
                            ...
                            <div class="form-check mb-2">
                                ...
                                <label class="form-check-label fw-bold" for="gstCheck">Apply GST % Optional</label>
                            </div>
                ...
                ?>
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h2>All Sales History & Reports</h2>
                    </div>
                    <div class="card card-box p-3 mb-3">
                        <form method="GET" class="row g-2">
                            ...
                            <div class="col-md-4">
                                <input type="date" name="date_filter" class="form-control" value="<?= $date_filter; ?>">
                            </div>
                            <div class="col-md-3">
                                <button type="submit" class="btn btn-primary w-100"><i class="fas fa-filter me-1"></i> Filter Sales</button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            <div class="card card-box p-3 bg-light border">
                                <span class="text-muted small fw-bold">TOTAL FILTERED UNITS SOLD</span>
                                <h5 class="fw-bold m-0 text-dark"><?= number_format($sales_totals['total_qty_sold'] ?? 0); ?> Units</h5>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="card card-box p-3 bg-light border">
                                <span class="text-muted small fw-bold">TOTAL FILTERED SALES INCOME</span>
                                <h5 class="fw-bold m-0 text-success">₹<?= number_format($sales_totals['total_sales_amt'] ?? 0, 2); ?></h5>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card card-box p-3">
                        <table class="table align-middle">
                            <thead><tr><th>Invoice</th><th>Customer</th><th>Product</th><th>Qty</th><th>Total Amount</th><th>Date</th><th>Action</th></tr></thead>
                            <tbody>
                                <?php 
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="index.php?page=sales&invoice=<?= $sl['invoice_no']; ?>" class="btn btn-sm btn-outline-secondary" title="View Bill"><i class="fas fa-eye"></i></a>
                                        <a href="index.php?page=all_sales&delete_sale=<?= $sl['id']; ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Delete sale record?')"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                                <?php endwhile; ?>
                            </tbody>
                ...
                ?>
                    ...
                    <div class="card card-box p-3 mt-3">
                        <form method="GET" class="row g-3 mb-3">
                            ...
                            <div class="col-md-4">
                                <select name="cat_filter" class="form-control" onchange="this.form.submit()">
                        ...
                        <table class="table table-bordered align-middle">
                            ...
                            <tbody>
                                ...
                                if(count($oos) > 0) {
                                    ...
                                    echo "<tr><td colspan='4' class='text-center py-4 text-muted'>No out of stock products found in this filter.</td></tr>";
                                }
                                ?>
                            </tbody>
                        </table>
    ...
    $(document).ready(function() {
        ...
        // Select2 inside modals with proper dropdownParent so live type searching works smoothly for Category & Supplier
